      ...
      <div className="relative w-full max-w-6xl h-[520px] sm:h-[580px] md:h-[650px] lg:h-[680px] flex items-center justify-center overflow-hidden py-4 select-none">
        <motion.div
          ...
          onDragEnd={(_, info) => {
            ...
            else if (info.offset.x > 30 || info.velocity.x > 200) handlePrev();
          }}
        >
          {events.map((evt, idx) => {
            const props = getMotionProps(idx);
            const poster = getEventPoster(evt);
            ...
            return (
              ...
              >
                ...
                {/* Title & Info */}
                <div
                  className="p-4 sm:p-5 flex-1 flex flex-col justify-center bg-white dark:bg-[#18181b] transition-opacity duration-200 min-h-[70px]"
                  style={{ opacity: props.isCenter ? 1 : 0.4 }}
                >
...
export default function EventsPage() {
  ...
  return (
    <main className="pt-28 md:pt-32 pb-16 px-6 max-w-7xl mx-auto w-full font-['Outfit'] bg-transparent relative z-10">
                  ...
                  <Link href={`/events/${evt.slug}`} key={`desktop-ongoing-${evt.id || evt.slug}`}>
                    <LiquidGlassCard className="h-[400px] flex overflow-hidden border-emerald-500/20 dark:border-emerald-500/30 group p-0" status="ongoing">
                      ...
                      <div className="w-1/2 relative bg-black/5 dark:bg-white/5 border-r border-black/5 dark:border-white/10">
                        {poster ? (
                          <Image src={poster} alt={evt.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
                        ) : (

...
export default function RoundCarousel({
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Calculate and report active item
      const currentActiveRaw = Math.round((-rotYRef.current / angle));
      const currentActive = ((currentActiveRaw % count) + count) % count;
      if (currentActive !== lastActiveRef.current) {
        lastActiveRef.current = currentActive;
        if (onActiveChangeRef.current) {
           onActiveChangeRef.current(currentActive);
        }
      }

...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0) {
    ...
    this.camera.position[2] = 3 / (scale || 1);
    this.#init(onInit);
  }
  ...
  #onControlUpdate(deltaTime) {
    ...
    let cameraTargetZ = 3 / (this.scaleFactor || 1);
      ...
      {activeItem && (
        <>
          ...
          <div className={`face-description ${isMoving ? 'inactive' : 'active'}`}>
            <h3 className="face-card-title">{activeItem.title}</h3>
            <p className="face-description-text">{activeItem.description}</p>
            {activeItem.tech && activeItem.tech.length > 0 && (
              <div className="face-tech-stack">
                {activeItem.tech.map((t, idx) => (
                  <span key={idx} className="face-tech-tag">{t}</span>
                ))}
              </div>
            )}
          </div>

...
function EventFallbackImage({ type }: { type: string }) {
  ...
  return (
    <div className={\`w-full h-full flex flex-col items-center justify-center \${bg} \${color}\`}>
      <Icon className="w-16 h-16 opacity-40 mb-3" strokeWidth={1.5} />
...
function CoverflowCarousel({ events }: { events: Event[] }) {
  ...
  return (
    <div className="w-full relative flex flex-col items-center">
      {/* Carousel Track Container - Tightly Composed */}
      <div className="relative w-full max-w-5xl h-[420px] sm:h-[480px] md:h-[510px] flex items-center justify-center overflow-hidden py-1 select-none">
        <motion.div 
          ...
          {events.map((evt, idx) => {
            ...
            return (
              <motion.div
                ...
                className={\`absolute w-[270px] sm:w-[380px] md:w-[440px] h-[370px] sm:h-[440px] md:h-[470px] rounded-2xl bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 overflow-hidden flex flex-col cursor-pointer \${
                  props.isCenter 
                    ? "shadow-[0_8px_24px_rgba(0,0,0,0.15)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.7)]" 
                    : "shadow-none"
                }\`}
              >
                {/* Poster Image / Fallback */}
                <div className={\`relative w-full overflow-hidden shrink-0 border-b border-black/5 dark:border-white/10 \${props.isCenter ? 'h-[75%]' : 'h-full'}\`}>
                  {evt.posterUrl ? (
                    <Image
                      src={evt.posterUrl}
                      alt={evt.title}
                      fill
                      sizes="(max-width: 640px) 270px, (max-width: 768px) 380px, 440px"
                ...
                {/* Title & Info - Shown for Center Poster Only */}
                {props.isCenter && (
                  <div className="p-4 sm:p-5 flex-1 flex flex-col justify-center bg-white dark:bg-[#18181b]">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="px-2.5 py-0.5 text-[9px] font-black uppercase tracking-widest rounded bg-black/5 dark:bg-white/10 text-[var(--text-secondary)]">
                        {evt.type || "Event"}
                      </span>
                    </div>
                    <h3 className="font-bold text-base sm:text-xl text-[var(--text-primary)] leading-snug line-clamp-1">
                      {evt.title}
                    </h3>
                  </div>
                )}
        ...
        <button
          ...
          className="absolute left-1 sm:left-4 z-40 p-2.5 sm:p-3 rounded-full bg-white/90 dark:bg-[#27272a]/90 text-[var(--text-primary)] border border-black/10 dark:border-white/10 hover:bg-white dark:hover:bg-[#27272a] transition-all shadow-md active:scale-95"
        >
        ...
        <button
          ...
          className="absolute right-1 sm:right-4 z-40 p-2.5 sm:p-3 rounded-full bg-white/90 dark:bg-[#27272a]/90 text-[var(--text-primary)] border border-black/10 dark:border-white/10 hover:bg-white dark:hover:bg-[#27272a] transition-all shadow-md active:scale-95"
        >
      ...
      {activeEvent && (
        <Link
          ...
          className="px-6 py-3 rounded-full bg-[var(--text-primary)] text-[var(--background)] font-bold text-sm hover:opacity-90 transition-opacity flex items-center gap-2 shadow-lg"
        >
...
export default function EventsPage() {
  ...
  return (
    <main className="min-h-screen pt-28 pb-20 px-6 max-w-7xl mx-auto w-full font-['Outfit']">
      ...
      {/* Featured Coverflow Carousel Section - Tightened Padding/Margins */}
...
console.log('Successfully fixed Coverflow Carousel sizing & spacing!');

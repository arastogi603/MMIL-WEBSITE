...
import { Calendar, Users, ChevronRight, ExternalLink, Code, Lightbulb, Palette, Trophy } from "lucide-react";
...
export default function EventsPage() {
  ...
  return (
    <main className="min-h-screen pt-32 pb-24 px-6 max-w-7xl mx-auto w-full font-['Outfit']">
      ...
      <div className="mb-16 text-center">
        <motion.h1 
          ...
          className="text-5xl md:text-7xl font-black mb-6 tracking-tight text-[var(--text-primary)] uppercase"
        >
          OUR EVENTS
        </motion.h1>
        <motion.p 
      ...
      {/* Featured Events Collage Section */}
      <section className="mb-24">
        ...
        {/* Desktop Layout: Overlapping Scattered Photo Collage */}
        <div className="hidden md:flex justify-center items-center py-14 px-4 overflow-visible min-h-[380px]">
          <div className="flex items-center justify-center -space-x-8 lg:-space-x-12">
            {featuredEvents.map((evt, idx) => {
              const styleConfig = collageStyles[idx % collageStyles.length];
              return (
                <div
                  key={\`collage-desktop-\${evt.id || evt.slug}\`}
                  className="relative group shrink-0 transition-[z-index] duration-0 group-hover:z-50"
                  style={{ zIndex: styleConfig.z }}
                >
                  <Link href={\`/events/\${evt.slug}\`} className="block">
                    <div
                      className="w-56 lg:w-64 h-80 rounded-2xl bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 overflow-hidden flex flex-col transition-all duration-250 ease-out group-hover:!rotate-0 group-hover:!translate-y-0 group-hover:scale-105 group-hover:shadow-[0_8px_20px_rgba(0,0,0,0.12)] dark:group-hover:shadow-[0_8px_25px_rgba(0,0,0,0.6)] cursor-pointer"
                      style={{
                        transform: \`rotate(\${styleConfig.rot}deg) translateY(\${styleConfig.y}px)\`,
                      }}
                    >
                      {/* Top ~70% Image/Icon block */}
                      <div className="relative w-full h-[70%] bg-black/5 dark:bg-white/5 overflow-hidden border-b border-black/5 dark:border-white/10 shrink-0">
                        {evt.posterUrl ? (
                          <Image
                            src={evt.posterUrl}
                            alt={evt.title}
                      ...
                      {/* Bottom ~30% Title block */}
                      <div className="p-4 flex-1 flex flex-col justify-center bg-white dark:bg-[#18181b]">
                        <h3 className="font-bold text-base lg:text-lg text-[var(--text-primary)] leading-snug line-clamp-2">
                          {evt.title}
                        </h3>
                      </div>
                    </div>
        ...
        <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-6 px-2 hide-scrollbar">
          {featuredEvents.map((evt) => (
            <div
              key={\`collage-mobile-\${evt.id || evt.slug}\`}
              className="snap-center shrink-0 w-64 h-80"
            >
              <Link href={\`/events/\${evt.slug}\`} className="block h-full">
                <div className="w-full h-full rounded-2xl bg-white dark:bg-[#18181b] border border-black/10 dark:border-white/10 overflow-hidden flex flex-col shadow-sm">
                  <div className="relative w-full h-[70%] bg-black/5 dark:bg-white/5 overflow-hidden border-b border-black/5 dark:border-white/10 shrink-0">
                    {evt.posterUrl ? (
                      <Image
                        src={evt.posterUrl}
...
console.log('Successfully updated src/app/(public)/events/page.tsx');
